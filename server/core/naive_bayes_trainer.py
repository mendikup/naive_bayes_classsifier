...
class NaiveBayesTrainer:
    ...
    def train_model(df):
        ...
        # Initialize the dictionary model with a nested dictionary called "sum"
        # to store counts needed for final probability calculations.
        bayes_model = {"sum": {"total_cases": total_rows}}

        for target_value in df[target_col].unique():
            # For each class label, count occurrences in the target column
            ...
            for feature in features:
                ...
                for unique_key in df[feature].unique():
                    ...
                    total_target_value_count = (
                        bayes_model['sum'][target_value] + num_unique_feature_values
                    )  # Includes Laplace
