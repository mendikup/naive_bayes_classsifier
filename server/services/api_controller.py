...
class ApiController:
    """
    API-facing controller that manages data loading, training, prediction, and exporting the latest trained model.
    This version includes an additional endpoint to fetch the last trained model and its metadata.
    """

    def __init__(self):
        """
        Initializes the API controller with empty state.
        """
        self.model = None
        self.accuracy = None
    ...
    def get_latest_model(self):
        """
        Returns the last trained model, its accuracy, and the feature set.
        This is useful for external services that need to sync or cache the latest model state.

        :return: A dictionary with the model, accuracy, and features (or indication if unavailable).
        """
        if not self.model:
            return {"exists": False}
        else:
